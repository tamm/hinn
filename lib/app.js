...
var app = angular.module('Hinn', [
    ...
    .run(['$rootScope', '$window', 'OAuth', '$mdToast', function($rootScope, $window, OAuth, $mdToast) {
        ...
        $rootScope.$on('oauth:error', function(event, rejection) {
            $rootScope.debugMessages.push({time: new Date(), line: 'oauth:error' + rejection});
            console.log('caught an error', rejection);
          // Ignore `invalid_grant` error - should be catched on `LoginController`.
          if ('invalid_grant' === rejection.data.error) {
            $rootScope.debugMessages.push({time: new Date(), line: 'invalid_grant'});
            return;
          }

          // Refresh token when a `invalid_token` error occurs.
          if ('invalid_token' === rejection.data.error && 'invalid_client' === rejection.data.error) {
            $rootScope.debugMessages.push({time: new Date(), line: 'invalid_token'});
          ...
          // return $window.location.href = '/login?error_reason=' + rejection.data.error;
            $rootScope.debugMessages.push({time: new Date(), line: 'didnt react to oauth:error at all'});
        });
    }])
    .controller('HinnController', [
        ...
        $rootScope.debugMessages.push({time: new Date(), line: 'started controller'});
